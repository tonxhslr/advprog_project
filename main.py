...
def timestep(params):
    ...
    Number of timesteps: params["nr_of_timesteps"]
    Start price: params["s_0"]
    Interest rate: params["r"]
    Dividend: params["q"]
    Volatility: params["iv"]
    '''
    delta_t=params["expiration"]/params["nr_of_timesteps"]
    return params["s_0"] * np.exp((params["r"]-params["q"]-0.5*params["iv"]**2)*delta_t + params["iv"] * np.sqrt(delta_t)*np.random.normal(0,1))
...
def MonteCarlo(params):
    ...
    Inputs:
    Number of simulations: params["nr_of_simulations"]
...
def payoff_barrier(path, params):
    ...
    then returns the vanilla payoff at expiry using the path’s last price if the knock condition is satisfied. 
    ...
    - params: dictionary of parameters, from which the function takes
        ...
        Option type: params["option_type"]
        Threshold K: params["threshold"]
        Barrier type: params["barrier_type"]
    '''
    ...
    european_payoff = max(ST - strike, 0.0) if params["option_type"] == "call" else max(strike - ST, 0.0)
...
Inputs:
 - params: dictionary of parameters, from which the function takes
     Excercise type (european, asian, etc.) : params["exercise_type"]
     Interest rate: params["r"]
     Expiration: params["expiration"]
